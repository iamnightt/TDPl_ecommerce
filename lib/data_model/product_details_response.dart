...
class DetailedProduct {
  ...
  // --- MOQ / SKU / Bulk pricing fields (see PRODUCT_PRICING_API.md) ---
  // Minimum Order Quantity: default qty and step size for the qty stepper.
  int? min_moq;
  // Bulk per-unit price (formatted) used at/above min_qty.
  String? min_qty_price;
  // Whether stock is tracked/limited (caps the Plus button).
  int? stock_manage;
  // Smallest sellable unit (floor). Not yet exposed by the detail API; parsed
  // tolerantly so it works once the backend adds it. Defaults handled in UI.
  int? sku_qty;
  // Per-unit price below min_moq (SKU tier). Not yet exposed by detail API.
  var sku_rate;
  // Quantity threshold where bulk pricing starts. Not yet exposed by detail API.
  int? min_qty;
  ...
  factory DetailedProduct.fromJson(Map<String, dynamic> json) =>
      DetailedProduct(
        ...
        min_moq: json["min_moq"] == null
            ? null
            : int.tryParse(json["min_moq"].toString()),
        min_qty_price: json["min_qty_price"]?.toString(),
        stock_manage: json["stock_manage"] == null
            ? null
            : int.tryParse(json["stock_manage"].toString()),
        sku_qty: json["sku_qty"] == null
            ? null
            : int.tryParse(json["sku_qty"].toString()),
        sku_rate: json["sku_rate"],
        min_qty: json["min_qty"] == null
            ? null
