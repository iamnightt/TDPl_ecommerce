...
import '../../helpers/moq_pricing_helper.dart';
...
class _ProductDetailsState extends State<ProductDetails>
  ...
  var _singlePrice;
  var _singlePriceString;
  // Per-unit rate for the active tier (sku/moq/bulk), from the price endpoint.
  String? _unitPriceString;
  int? _quantity = 1;
  int? _stock = 0;
  var _stock_txt;

  // --- MOQ / SKU stepper state (see PRODUCT_PRICING_API.md §5) ---
  int _minMoq = 1;
  int _skuQty = 1;
  bool _stockManaged = false;
  ...
  setProductDetailValues() {
    if (_productDetails != null) {
      ...
      // MOQ / SKU stepper config (PRODUCT_PRICING_API.md §5). The qty selector
      // starts at min_moq and steps by min_moq, with sku_qty as the floor.
      _minMoq = MoqPricing.effectiveMinMoq(_productDetails!.min_moq);
      _skuQty = MoqPricing.effectiveSkuQty(_productDetails!.sku_qty, _minMoq);
      _stockManaged = (_productDetails!.stock_manage ?? 0) == 1;
      _quantity = _minMoq;
      _productDetails!.photos!.forEach((photo) {
        _productImageList.add(photo.path);
      });
  ...
  fetchAndSetVariantWiseInfo({bool change_appbar_string = true}) async {
    ...
    // Refresh MOQ config from the per-variant response when present (it can
    // differ per variant). Falls back to the detail-level values.
    if (variantResponse.variantData!.minMoq != null) {
      _minMoq = MoqPricing.effectiveMinMoq(variantResponse.variantData!.minMoq);
      _skuQty = MoqPricing.effectiveSkuQty(_productDetails?.sku_qty, _minMoq);
    }
    if (variantResponse.variantData!.stockManage != null) {
      _stockManaged = variantResponse.variantData!.stockManage == 1;
    }

    // Keep quantity within [skuQty, stock] (only cap by stock when managed).
    _quantity = MoqPricing.clampManual(
      _quantity ?? _minMoq,
      minMoq: _minMoq,
      skuQty: _skuQty,
      stockManaged: _stockManaged,
      maxLimit: _stock,
    );

    _variant = variantResponse.variantData!.variant;
    _totalPrice = variantResponse.variantData!.price;
    _unitPriceString = variantResponse.variantData!.unitPrice;
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_productDetails != null &&
                              _unitPriceString != null) ...[
                            Padding(
                              padding:
                                  const EdgeInsets.symmetric(horizontal: 4),
                              child: buildUnitRateRow(),
                            ),
                            SizedBox(height: 10),
                          ],
                          Padding(
  ...
  // Per-unit rate for the active tier (sku/moq/bulk) returned by the price
  // endpoint. Updates whenever quantity / variant changes (PRODUCT_PRICING_API.md §5).
  Widget buildUnitRateRow() {
    final unit = _productDetails?.unit;
    return Row(
      children: [
        Padding(
          padding: app_language_rtl.$!
          ...
          child: Container(
            width: 75,
            child: Text(
              AppLocalizations.of(context)!.unit_price_ucf,
              style: TextStyle(color: Color(0xff6B7377), fontSize: 10),
            ),
          ),
        ),
        Padding(
          ...
          child: Text(
            (SystemConfig.systemCurrency != null
                    ? _unitPriceString.toString().replaceAll(
                        SystemConfig.systemCurrency!.code!,
                        SystemConfig.systemCurrency!.symbol!)
                    : _unitPriceString.toString()) +
            ...
            children: [
              ...
              Container(
                  ...
                  child: Center(
                      child: QuantityInputField.show(quantityText,
                          isDisable: _quantity == 0, onSubmitted: () {
                    // Clamp a manually typed value into [sku_qty, stock] (§5).
                    final typed = int.tryParse(quantityText.text) ?? _minMoq;
                    _quantity = MoqPricing.clampManual(
                      typed,
                      minMoq: _minMoq,
                      skuQty: _skuQty,
  ...
  buildQuantityUpButton() => Container(
        ...
        child: IconButton(
            icon: Icon(Icons.add, size: 16, color: MyTheme.dark_grey),
            onPressed: () {
              // §5 Plus: jump SKU->MOQ, else step by min_moq; cap at stock.
              if (!MoqPricing.canIncrement(_quantity ?? _minMoq,
                  stockManaged: _stockManaged, maxLimit: _stock)) {
                return;
              }
              final next = MoqPricing.increment(
                _quantity ?? _minMoq,
                minMoq: _minMoq,
                stockManaged: _stockManaged,
                maxLimit: _stock,
  ...
  buildQuantityDownButton() => Container(
      ...
      child: IconButton(
          icon: Center(
              child: Icon(Icons.remove, size: 16, color: Color(0xff707070))),
          onPressed: () {
            // §5 Minus: step down by min_moq, drop to sku_qty at the MOQ floor;
            // disabled at/below sku_qty.
            if (!MoqPricing.canDecrement(_quantity ?? _minMoq,
                minMoq: _minMoq, skuQty: _skuQty)) {
              return;
            }
            final next = MoqPricing.decrement(
              _quantity ?? _minMoq,
              minMoq: _minMoq,
              skuQty: _skuQty,
            );
